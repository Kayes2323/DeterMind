...
const SAMPLE_PROMPT_BN = `আমি HSC 2026 পরীক্ষার্থী। Physics আর Math দুর্বল। সকাল ৬টায় উঠি, রাত ১১টায় ঘুমাই। ৫ ওয়াক্ত নামাজ পড়ি।`
const SAMPLE_PROMPT_EN = `I'm an HSC 2026 student. Physics and Math are weak. I wake at 6am, sleep at 11pm. I pray 5 times.`

function RoutineRow({ row, onUpdate, onDelete, color }) {
  ...
  const generateRoutine = async () => {
    ...
    try {
      const systemPrompt = lang === 'bn'
        ? `তুমি একজন বিশেষজ্ঞ শিক্ষার্থী কোচ। ব্যবহারকারীর তথ্য বিশ্লেষণ করে একটি বিজ্ঞানসম্মত দৈনিক রুটিন তৈরি করো। JSON ফরম্যাটে দাও। শুধু JSON, আর কিছু না।
Format: { "title": "রুটিনের নাম", "goal": "লক্ষ্য", "rows": [{"time": "৬:০০ AM", "task": "কাজের বিবরণ", "duration": "৩০ মিনিট", "category": "prayer|study|break|exercise|sleep"}] }`
        : `You are an expert student coach. Analyze the user's info and create a science-based daily routine. Return only JSON.
Format: { "title": "Routine Name", "goal": "Goal", "rows": [{"time": "6:00 AM", "task": "Task description", "duration": "30 min", "category": "prayer|study|break|exercise|sleep"}] }`

      const text = await callClaude([{ role: 'user', content: prompt }], systemPrompt)
      const clean = text.replace(/```json|```/g, '').trim()
      const parsed = JSON.parse(clean)
      setRoutine(parsed)
    } catch (e) {
      setError(t(lang,'AI কানেকশন সমস্যা। API key সেট করা আছে কি?','AI connection error. Is the API key set?'))
    } finally {
      setLoading(false)
    }
  }
  ...
  const categoryColors = { prayer:'text-green-400', study:'text-blue-400', break:'text-yellow-400', exercise:'text-orange-400', sleep:'text-purple-400' }
    ...
    <div className="pb-20 md:pb-6">
      ...
      {!routine && (
        <Card className="mb-5 border border-orange-500/20">
          ...
          <textarea
            ...
            rows={4}
            placeholder={lang==='bn' ? SAMPLE_PROMPT_BN : SAMPLE_PROMPT_EN}
          ...
          <Button onClick={generateRoutine} disabled={loading || !prompt.trim()} className="w-full" size="lg">
            {loading ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/> {t(lang,'তৈরি হচ্ছে...','Generating...')}</> : <><Wand2 size={16}/> {t(lang,'রুটিন তৈরি করো','Generate Routine')}</>}
          </Button>
        </Card>
      )}
      ...
      {routine && (
        <>
          ...
          {/* Printable area */}
          <div ref={printRef} className="print:bg-white print:p-8">
            ...
            <div className="hidden print:block">
              ...
              <p className="text-xs text-gray-400 mt-4">Generated by DeterMind — Track Today. Rise Tomorrow.</p>
            </div>
          </div>
