...
export default function Layout({ children }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-dark-900">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-30 glass border-t border-white/5 flex justify-around px-1 py-1.5">
        {navItems.map(({ to, icon: Icon, label, labelEn }) => (
          <NavLink key={to} to={to} end={to === '/'}
            className={({ isActive }) => clsx(
              'flex flex-col items-center gap-0.5 px-2 py-1 rounded-xl text-[9px] transition-all',
